...
### Pythonの名前付きパラメータ（キーワード引数）について
- Pythonは関数呼び出し時に「名前付きパラメータ（キーワード引数）」が使えるのがとても便利！
- Kotlinでも同じような機能があって好きだったので、Pythonでも同じ感覚で書けるのが嬉しい。
- 可読性も上がるし、引数の順番を気にせず明示的に渡せるのが最高。

### Pythonのデフォルト引数について
- Pythonは関数定義時にデフォルト引数（デフォルトパラメータ）が書けるのが便利！
- 必須の引数と任意の引数を柔軟に組み合わせられるので、関数の使い勝手が良くなる。
- Kotlinでも同じような仕組みがあって好きだったので、Pythonでも重宝してます。

### Pythonのfor-else構文について
- Python独自の構文で、for文のループが「breakせずに最後まで回りきった場合」にだけelse節が実行される。
- 何かを探索して見つかったらbreak、見つからなければelseで「見つからなかった」処理を書くのに便利。
- 最初は戸惑うけど、慣れると意外と使いどころがある。
